...
| `statusUrl` | `String?` | production API | Optional HTTPS API endpoint on the `devguard.uk` domain (for example `https://api.devguard.uk/devguard`). |
...
5. Tap it and enter your passcode to view device telemetry and local diagnostic logs.
...
1. **Release hardening (your app)**: Always build release with `--obfuscate --split-debug-info=./debug_info`:
     ```bash
